...
function CommunityWritePage() {
  ...
  const lastAutoGeneratedId = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const newTitle = `${selectedRoadmap.targetJob} 로드맵을 공유합니다`
    const newSummary = `${selectedRoadmap.targetJob} 준비 과정에서 정리한 학습 로드맵입니다.`
    const shouldUpdateTitle =
      !title.trim() || lastAutoGeneratedId.current !== selectedRoadmap.id || title === lastAutoTitle.current
    const shouldUpdateSummary =
      !roadmapSummary.trim() || lastAutoGeneratedId.current !== selectedRoadmap.id || roadmapSummary === lastAutoSummary.current
    ...
    if (shouldUpdateSummary) {
      setRoadmapSummary(newSummary)
    }

    lastAutoGeneratedId.current = selectedRoadmap.id
    lastAutoTitle.current = newTitle
    lastAutoSummary.current = newSummary
